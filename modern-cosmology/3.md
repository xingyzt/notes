...
To solve this equation for the FRLW metric, we note that every real particle’s comoving momentum 
$P^\mu = \f{\d x^\mu}{\d\lambda}$ obeys the **mass-shell constraint**
\\[
P^2 \equiv g\_{\mu\nu} P^\mu P^\nu = -E^2 + p^2 = -m^2,
\tag{3.27}
\\]
where, in the FRLW metric, the physical momentum has magnitude $p \equiv \sqrt{g_{ij} P^i P^j}$ and unit vectors $\hat p^i$, related to the comoving momentum by
\\[
P^i = \f{p\hat p^i}a.
\tag{3.32}
\\]
...
Inhomogeneities and anisotropies in spacetime, either due to uneven distributions of mass-energy, 
or due to gravitational waves in spacetime itself, are reflected in perturbations $h\_{\mu\nu}$ to the FLRW metric $g^{(0)}\_{\mu\nu}$.
...
while scalar perturbations correspond to simple spacetime curvature, 
