...
Subtracting out $C^{(0)}[f]$ from [(3.38)](#tag-3.38) gives the perturbation term
\\[
\delta C[f]
= - \brk{ \f E{ap} [\uvec p\cdot\nabla]\Psi + \dot\Phi }p \f{\p f}{\p p}
\\]
In the relativistic limit ($E\simeq p$), this reduces to
\\[
\delta C[f]
\simeq - \brk{ \f 1a [\uvec p\cdot\nabla]\Psi + \dot\Phi } p \f{\p f}{\p p},
\tag{3.74}
\\]
while in the non-relativistic limit ($E \simeq m$), this reduces to
\\[
\delta C[f]
\simeq - \brk{ \f m{ap} [\uvec p\cdot\nabla]\Psi + \dot\Phi } p \f{\p f}{\p p}.
    \tag{3.76}
\\]
