...
But because all real particles obey the dispersion relation $-m\_s^2 = g_{\mu\nu} P^\mu P^\nu = -E\_s^2 + p^2$,
for any given rest mass $m\_s$, 
...
the relativistically invariant fundamental integration measure is thus
\\[
\f{\d^3 p}{[2\pi]^3 \cdot 2E(p)}.
\\]

At high densities, the differing spin statistics of bosonic and fermionic particles start to significantly affect
their distributions as well.
