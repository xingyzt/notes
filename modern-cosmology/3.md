...
\tag{3.27}
\\]
where, in the FRLW metric, the physical momentum has magnitude $p \equiv \sqrt{g_{ij} P^i P^j}$ and unit vectors $\hat p^i$, related to the four-momentum by
\\[
P^i = \f{p\hat p^i}a.
\tag{3.32}
\\]
...
A collision is an umbrella term for any sort of direct particle interaction, 
including scattering, pair production, annihilation, and decay. 
Consider, for instance, a reversible reaction
\\[
(1)\_{\vec p} + (2)\_{\vec q}
\leftrightarrow (3)\_{\vec p'} + (4)\_{\vec q'}
\\]
between species 1, 2, 3, and 4, with physical momenta $\vec p$, $\vec q$, $\vec p'$, and $\vec q'$ respectively.
