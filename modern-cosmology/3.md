...
\tag{3.66}
\end{aligned}\\]
where $\nabla\_\parallel \equiv \uvec p[\uvec p\cdot\nabla]$ gives the gradient vector in the direction of motion.
...
\tag{3.72}
\end{aligned}\\]
Notice how the curvature in $\Psi$ deflect nonrelativistic particles ($p \ll E$)
significantly more than relativistic particles ($p \simeq E$),
while the curvature in $\Phi$ only significantly deflect relativistic particles.
Because $\Phi \simeq -\Phi$, 
...
In the original FLRW metric,
\\[
\f{\p f}{\p\vec p}
\\]
In the perturbed spacetime, 
the distribution function may have small anisotropies $\p f / \p\uvec p$ 
of the same order as the metric perturbations.
Approximating to linear order in all these perturbations, we have
\\[\begin{aligned}
\f{\d f}{\d t}
&= \f{\p f}{\p t} + \f{\p f}{\p\vec x} \cdot \f{\d\vec x}{\d t} + \f{\p f}{\p\vec p} \cdot \f{\d\vec p}{\d t} 
\\\
