...
tells us that we cannot distinguish phase-space coordinates less than
$
\Delta x \Delta p < [2\pi]^3
$
in separation. (Recall that we work in units where $\hbar = 1$.)
Thus, every such phase-space volume in contains
\\[
...
Consider a particle with rest energy $E\_0$, located at position $\vec x\_0$. 
In the rest frame, we can always find a Minkowskian coordinate system (by the local flatness theorem)
such that the metric is $g\_{\mu\nu} = \eta\_{\mu\nu}$.
In this coordinate system, particle's relativistic momentum is $P^\mu \to (E\_0, 0, 0, 0)$, 
and its energy-momentum tensor field surrounding a physical-space volume $\d\mathcal V = \d^3 x$ about $\vec x\_0$ is
\\[
...
\\[
T{^\mu}\_{\nu}(\vec x\_0, t)\, \d\mathcal V = \f{P^\mu(t) P\_\nu(t)}{E\_0}.
\\]
...
\\]
where the temporal component of relativistic momentum transforms as $P^0 = \gamma E\_0$.
...
\tag{3.20}
\\]
As corollaries, the comoving mass density is then
\\[
\rho\_s(\vec x, t)
