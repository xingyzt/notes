...
\tag{2.26}
\\]
This relation in fact holds in the massless limit as well, 
when $\tau/m$ converges to the comoving time $x^0 = t$.
...
In general, the density of a medium with a constant **equation of state** $w \equiv \mathcal P / \rho$ evolves as $\rho \propto a^{-3-3w}$. For radiation, $\mathcal P\_r \simeq \rho\_r/3 \implies \rho\_r \propto a^{-4}$, whereas for nonrelativistic matter, $\mathcal P\_m \simeq 0 \implies \rho\_m \propto a^{-3}$. The cosmological constant, with $w\_\Lambda = -1$, is thus constant through time.
...
\\[\\begin{aligned}
\hat A\_s(\vec x, t) &= g\_s \int \f{\d^3 p}{[2\pi]^3}\, f\_s(\vec x, \vec p, t) A\_s(\vec x, \vec p)
\\end{aligned}\\]
...
with the comoving momentum $P^\alpha = (E, \vec p/a)$ in a reference frame which flows locally with the particles.The comoving mass density is then
\\[
\rho\_s(\vec x, t)
...
For photons and neutrinos, $\mu\_r \ll T$.
Evaluating $\mathcal{P}\_r$ in thermal equilibrium thus gives
\\[
\f{\p\mathcal{P}\_r}{\p T} = \f{\rho\_r + \mathcal{P}\_r}{T} \propto a^{-3}.
\tag{2.67}
\\]
...
\\[
\Omega\_s \equiv \f{\rho\_s(t\_0)}{\rho\_{cr}},
\tag{2.71}
\\]
i.e. its present-day density as a fraction of the universe’s critical density $\rho\_{cr}$, we can write the physical density as 
\\[
\rho\_s(a) = \Omega\_s \rho\_{cr} a^{-3[1 + w\_s]},
\tag{2.72}
\\]
...
\\[
\Omega\_b = 0.0222(5) h^{-2},\, 0.0225(3) h^{-2}
\\]
respectively. Thus $\rho\_b \simeq 0.05\rho\_{cr}$, hinting a significant amount of non-baryonic matter.
